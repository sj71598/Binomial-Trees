# CRR Fuction
CRRfunction <- function (TypeFlag = c("ce", "pe", "ca", "pa"), S, X, Time, 
          r, b, sigma, n, title = NULL, description = NULL) 
{
  TypeFlag = TypeFlag[1]
...
      Tree <- c(Tree, rep(0, times = n - j))
      for (i in 0:j) {
        OptionValue[i + offset] = max((z * (S * u^i * 
                                              d^(abs(i - j)) - X)), (p * OptionValue[i + 
                                                                                       1 + offset] + (1 - p) * OptionValue[i + offset]) * Df)
        Tree = c(Tree, OptionValue[i + offset])
      }
    }
  }
...
#LogNormal
LogNfunction <- function (TypeFlag = c("ce", "pe", "ca", "pa"), S, X, Time, 
          r, b, sigma, n, title = NULL, description = NULL) 
{
  TypeFlag = TypeFlag[1]
  if (TypeFlag == "ce" || TypeFlag == "ca") 
...
    z = -1
  dt = Time/n
  u = exp(((r-(.5*(sigma^2)))*dt)+(sigma * sqrt(dt)))
  d = exp(((r-(.5*(sigma^2)))*dt)-(sigma * sqrt(dt)))
  p = (exp(b * dt) - d)/(u - d)
  Df = exp(-r * dt)
  OptionValue = z * (S * u^(0:n) * d^(n:0) - X)
  offset = 1
